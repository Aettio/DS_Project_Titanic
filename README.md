...
Я добавил ещё пару строк для визуализации самого дерева, чтобы всё было понятней и вот что получилось. Процент предсказания довольно не плохой, учитывая что это дерево можно использовать для анализа любых случайно взятых данных из титаника. Я старался найти оптимальный баланс между его размерами чтобы дерево не было переобученым. Как итог получилось ≈80% или 4 из 5 алгоритм классифицирует правильно. Можно добиться и более высоких значний классификации, но тогда алгоритм будем менее универсальным.
